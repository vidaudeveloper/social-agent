...
| **X (Twitter)** | baoyu-post-to-x (Chrome CDP) | ✅ 填稿已测试通过（默认不点 Post） |
...
npm run x:setup
$env:OVERSEAS_ALLOW_AUTOMATION="true"
npm run x:login          # 只开一次登录页，登录完 Enter
npm run x:publish -- --text "测试文案"   # 默认仅填稿，不点 Post
```

上游 clone 到 `tool/baoyu-skills-vendor`（不完整 clone 时自动 fallback；`BAOYU_SKILLS_ROOT` 可覆盖）。Chrome 配置目录：`%APPDATA%\baoyu-skills\chrome-profile`。
