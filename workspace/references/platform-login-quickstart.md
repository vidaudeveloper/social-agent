...
| **LinkedIn** | `linkedin:setup` + `.env` OAuth | 授权页 **手动登录授权** → 终端 **Enter**（当前暂缓自动发） | `npm run linkedin:check-login` |
| **X** | baoyu-post-to-x | Chrome **手动登 X**；默认填稿后你在浏览器点 Post | `node skills/x/scripts/cli.mjs check-login` |
...
默认只出稿。`node skills/x/scripts/cli.mjs login` → Chrome 登录 X → 发布时在浏览器确认 Post。
