...
认证：`Authorization: Bearer {OPENAI_API_KEY}`（项目 `.env`，`项目 .env 路径`）

## CLI（推荐）

```powershell
npm run image:generate -- --platform zhihu --prompt "..." --out "$CONTENT_ROOT/图片/知乎/cover.png"
...
- URL 有时效，生成后立即下载到 `$CONTENT_ROOT/图片/{平台}/`
- timeout ≥ 120 秒
