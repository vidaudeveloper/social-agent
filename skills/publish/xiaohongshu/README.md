# skills/publish/xiaohongshu

小红书自动化 Skills，直接使用你已登录的浏览器和真实账号，以普通用户的方式操作小红书。

支持 [VidAU social-agent](https://github.com/vidaudeveloper/social-agent) 及所有兼容 `SKILL.md` 格式的 AI Agent 平台。
...
| **xhs-post-analytics** | 发后作品复盘 | 自己账号作品数据 → HTML + 下次创作参考 | `analytics/xhs-post-analytics`（`npm run xhs:stats`） |
...
本技能已捆绑在 [social-agent](https://github.com/vidaudeveloper/social-agent) profile 中，路径为 `skills/publish/xiaohongshu/`，通过 VidAU 安装 profile 后即可使用，**无需单独 clone 其他仓库**。
...
MIT（上游组件见仓库根目录 [NOTICE.md](../../../../NOTICE.md)）。
